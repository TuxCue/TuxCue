...
Normal FUSE execution could not be tested in the execution environment because `/dev/fuse` is unavailable. The AppImage's extract-and-run mode was used for the packaged tests. GitHub Actions has not run remotely; the workflow is prepared for source/browser checks only.
