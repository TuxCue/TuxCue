...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='upgrade-',) as folder:
        ...
        try:
            ...
            assert old_audio
            new=launch('0.4.5');after=ready('0.4.5',connected=True)
            ...
            passed.extend(['0.4.5 cleanly replaces a running 0.4.2 AppImage','Collection, set, original audio and settings survive',
                           'Connected virtual microphone restored after old owned devices are released','Paired phone reconnects without pairing again'])
            duplicate=launch('0.4.5');duplicate.wait(timeout=10);assert duplicate.returncode==0
            ...
            duplicate=launch('0.4.5',{**env,'PATH':str(stub)+os.pathsep+env['PATH'],'TUXCUE_TEST_BROWSER_URL':str(root/'opened-url')},browser=True)
            ...
            shutdown(new)
            racers=[launch('0.4.5') for _ in range(3)]
            fresh=ready('0.4.5',connected=False)
